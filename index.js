...
bot.onText(/\/stop/, (msg) => {
  ...
  saveUsers();
  bot.sendMessage(msg.from.id, `👋 Ты отписан от рассылки.`);
});
...
// ── API ──
app.get('/api/status', (req, res) => res.json({ ok: true, users: users.size }));

app.get('/api/users', (req, res) => {
  const list = [...users.values()].map(u => ({
    id: u.id, name: u.name, username: u.username, date: u.date,
      ...
      try {
        if (files.length === 0) {
          ...
          for (let i = 0; i < files.length; i++) {
            ...
            if (f.mimetype.startsWith('image/')) await bot.sendPhoto(uid, f.path, { caption, parse_mode: 'HTML' });
            else await bot.sendDocument(uid, f.path, { caption, parse_mode: 'HTML' });
          }
        }
        sent++;
...
app.listen(PORT, () => {
  console.log(`🚀 BlastSend запущен на порту ${PORT}`);
